...
public class ButtonsViewComponent : NopViewComponent
{
    ...
    public async Task<IViewComponentResult> InvokeAsync(string widgetZone, object additionalData)
    {
        ...
        if (!await _paymentPluginManager.IsPluginActiveAsync(TamaraDefaults.SystemName, customer, store?.Id ?? 0))
        ...
        if (widgetZone.Equals(PublicWidgetZones.OrderSummaryContentAfter))
        {
            ...
            if (routeName != TamaraDefaults.ShoppingCartRouteName)
                return Content(string.Empty);
        }
        ...
        return View("~/Plugins/Payments.Tamara/Views/Buttons.cshtml", (widgetZone, productId, productCost));
    }
