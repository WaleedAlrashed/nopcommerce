...
public class ServiceManager
{
    ...
    protected readonly TamaraHttpClient _onboardingHttpClient;
    ...
    public ServiceManager(CurrencySettings currencySettings,
        ...
        TamaraHttpClient onboardingHttpClient)
    {
    ...
    protected async Task<(TResult Result, string Error)> HandleFunctionAsync<TResult>(Func<Task<TResult>> function)
    {
        ...
        catch (Exception exception)
        {
            ...
            //log errors
            var logMessage = $"{TamaraDefaults.SystemName} error: {System.Environment.NewLine}{message}";
    ...
    /// </returns>
    protected static async Task<TResult> HandleCheckoutRequestAsync<TRequest, TResult>(TamaraSettings settings, TRequest request)
        where TRequest : HttpRequest where TResult : class
    {
        //prepare common request params
        request.Headers.Add(HeaderNames.UserAgent, TamaraDefaults.UserAgent);
        request.Headers.Add("PayPal-Partner-Attribution-Id", TamaraDefaults.PartnerCode);
    ...
    /// The task result contains the result
    /// </returns>
    protected static async Task<TResult> HandleCoreRequestAsync<TRequest, TResult>(TamaraSettings settings, TRequest request)
    ...
    /// </returns>
    public async Task<(AccessToken AccessToken, string Error)> GetAccessTokenAsync(TamaraSettings settings)
    {
    ...
    /// </returns>
    public async Task<(string Script, string Error)> GetScriptAsync(TamaraSettings settings, string widgetZone)
    {
        return await HandleFunctionAsync(async () =>
        {
            ...
            var scriptUrl = QueryHelpers.AddQueryString(TamaraDefaults.ServiceScriptUrl, parameters);
            ...
            return $@"<script src=""{scriptUrl}"" data-partner-attribution-id=""{TamaraDefaults.PartnerCode}"" data-page-type=""{pageType}""></script>";
        });
    }
    ...
    /// </returns>
    public async Task<(Order Order, string Error)> CreateOrderAsync(TamaraSettings settings, Guid orderGuid)
    {
        return await HandleFunctionAsync(async () =>
        {
            //ensure that plugin is configured
            ...
            PayPalCheckoutSdk.Orders.Money prepareMoney(decimal value) => new()
            {
                ...
                Value = value.ToString(TamaraDefaults.CurrenciesWithoutDecimals.Contains(currency.ToUpperInvariant()) ? "0" : "0.00", CultureInfo.InvariantCulture)
            };
    ...
    /// </returns>
    public async Task<(Order Order, string Error)> AuthorizeAsync(TamaraSettings settings, string orderId)
    {
        return await HandleFunctionAsync(async () =>
        {
            //ensure that plugin is configured
    ...
    /// </returns>
    public async Task<(Order Order, string Error)> CaptureAsync(TamaraSettings settings, string orderId)
    {
        return await HandleFunctionAsync(async () =>
        {
            //ensure that plugin is configured
    ...
    public async Task<(PayPalCheckoutSdk.Payments.Capture Capture, string Error)> CaptureAuthorizationAsync
        (TamaraSettings settings, string authorizationId)
    {
        return await HandleFunctionAsync(async () =>
        {
            //ensure that plugin is configured
    ...
    /// </returns>
    public async Task<(object Order, string Error)> VoidAsync(TamaraSettings settings, string authorizationId)
    {
        return await HandleFunctionAsync(async () =>
        {
            //ensure that plugin is configured
    ...
    public async Task<(PayPalCheckoutSdk.Payments.Refund Refund, string Error)> RefundAsync
        (TamaraSettings settings, string captureId, string currency, decimal? amount = null)
    {
        return await HandleFunctionAsync(async () =>
        {
            //ensure that plugin is configured
            ...
            if (amount.HasValue)
            {
                ...
                {
                    CurrencyCode = currency,
                    Value = amount.Value.ToString(TamaraDefaults.CurrenciesWithoutDecimals.Contains(currency.ToUpperInvariant()) ? "0" : "0.00", CultureInfo.InvariantCulture)
                };
            }
    ...
    /// </returns>
    public async Task<(Webhook Webhook, string Error)> GetWebhookAsync(TamaraSettings settings, string webhookUrl)
    {
        return await HandleFunctionAsync(async () =>
        {
            //ensure that plugin is configured
    ...
    /// The task result contains the webhook; error message if exists
    /// </returns>
    public async Task<(Webhook Webhook, string Error)> CreateWebhookAsync(TamaraSettings settings, int storeId)
    {
        return await HandleFunctionAsync(async () =>
        {
            //ensure that plugin is configured
            ...
            var webhookUrl = $"{store.Url.TrimEnd('/')}{urlHelper.RouteUrl(TamaraDefaults.WebhookRouteName)}".ToLowerInvariant();
            ...
            {
                EventTypes = TamaraDefaults.WebhookEventNames.Select(name => new EventType { Name = name }).ToList(),
                Url = webhookUrl
            });
    ...
    /// <returns>A task that represents the asynchronous operation</returns>
    public async Task HandleWebhookAsync(TamaraSettings settings, Microsoft.AspNetCore.Http.HttpRequest request)
    {
        await HandleFunctionAsync(async () =>
        {
            //ensure that plugin is configured
            ...
            {
                case "completed":
                    var refundIds = await _genericAttributeService.GetAttributeAsync<List<string>>(order, TamaraDefaults.RefundIdAttributeName)
                                    ?? new List<string>();
                    if (!refundIds.Contains(refund.Id))
                    {
                        if (decimal.TryParse(refund.Amount?.Value, out var refundedAmount))
                        {
                            if (_orderProcessingService.CanPartiallyRefundOffline(order, refundedAmount))
                            {
                                ...
                                await _genericAttributeService.SaveAttributeAsync(order, TamaraDefaults.RefundIdAttributeName, refundIds);
                            }
                        }
                    }
                    break;
    ...
    public async Task<(Merchant Merchant, string Error)> OnboardAsync(string email)
    {
        return await HandleFunctionAsync(async () =>
        {
            ...
            var store = storeId > 0
                ...
                : await _storeContext.GetCurrentStoreAsync();
            var urlHelper = _urlHelperFactory.GetUrlHelper(_actionContextAccessor.ActionContext);
            var redirectUrl = urlHelper.RouteUrl(TamaraDefaults.ConfigurationRouteName, null, _webHelper.GetCurrentRequestProtocol());
