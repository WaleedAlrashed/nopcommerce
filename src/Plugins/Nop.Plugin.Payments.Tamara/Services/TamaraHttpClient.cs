...
namespace Nop.Plugin.Payments.Tamara.Services
{
    public class TamaraHttpClient
    {
        ...
        {
            //configure client
            httpClient.BaseAddress = new Uri(TamaraDefault.ServiceUrl);
            httpClient.Timeout = TimeSpan.FromSeconds(TamaraDefault.RequestTimeout);
            httpClient.DefaultRequestHeaders.Add(HeaderNames.UserAgent, TamaraDefault.UserAgent);
        ...
        public async Task<TResponse> RequestAsync<TRequest, TResponse>(TRequest request, TamaraSettings settings) where TRequest : Request where TResponse : class
        {
            try
            {
                ...
                requestMessage.Headers.Add("Authorization", $"Bearer {settings.ApiToken}");
