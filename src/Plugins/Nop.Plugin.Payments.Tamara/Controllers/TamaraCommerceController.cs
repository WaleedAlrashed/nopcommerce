...
public class TamaraController : BasePluginController
{
    ...
    protected async Task PrepareCredentialsAsync(ConfigurationModel model, TamaraSettings settings, int storeId)
    {
        ...
        {
            var locale = await _localizationService.GetResourceAsync("Plugins.Payments.Tamara.Configuration.Error");
        ...
        {
            var onboardingNotCompleted = await _localizationService.GetResourceAsync("Plugins.Payments.Tamara.Onboarding.InProcess");
        ...
        {
            var onboardingError = await _localizationService.GetResourceAsync("Plugins.Payments.Tamara.Onboarding.Error");
            _notificationService.ErrorNotification(onboardingError);
            return;
        }

        var onboardingCompleted = await _localizationService.GetResourceAsync("Plugins.Payments.Tamara.Onboarding.Completed");
        ...
        if (!string.IsNullOrEmpty(settings.ClientId) && !string.IsNullOrEmpty(settings.SecretKey))
        {
            ...
            if (string.IsNullOrEmpty(settings.WebhookUrl))
            {
                ...
                var warning = string.Format(await _localizationService.GetResourceAsync("Plugins.Payments.Tamara.WebhookWarning"), url);
    ...
    public async Task<IActionResult> Configure(bool showtour = false)
    {
        ...
        if (storeId > 0)
        {
            ...
            settings.ClientId = await _settingService
                .GetSettingByKeyAsync<string>($"{nameof(TamaraSettings)}.{nameof(TamaraSettings.ClientId)}", storeId: storeId);
    ...
    public async Task<IActionResult> Onboarding(OnboardingModel model)
    {
        ...
        var emailSet = await _localizationService.GetResourceAsync("Plugins.Payments.Tamara.Onboarding.EmailSet");
    ...
    public async Task<IActionResult> RevokeAccess()
    {
        ...
        var accessRevoked = await _localizationService.GetResourceAsync("Plugins.Payments.Tamara.Onboarding.AccessRevoked");
